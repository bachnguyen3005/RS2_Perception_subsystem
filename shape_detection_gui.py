    ...
    def detect_shapes(self):
        ...
        try:
            ...
            if (self.roi_start_x is not None and self.roi_end_x is not None and 
                ...
                img_text = f"Full image size: {full_width}x{full_height}"
                cv2.putText(self.result_image, img_text, (10, full_height - 15), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                            
            else:
                # Process the entire image - no ROI offset needed
                self.result_image = detect_shapes_and_colors(image_to_process, target)
